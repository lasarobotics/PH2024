...
public class RobotContainer {
  ...
  // private static final ShooterSubsystem SHOOTER_SUBSYSTEM = new
  // ShooterSubsystem(
  ...
  public RobotContainer() {
    ...
    // NamedCommands.registerCommand(Constants.NamedCommands.INTAKE_COMMAND_NAME,
    // intakeCommand().withTimeout(2));
    // NamedCommands.registerCommand(Constants.NamedCommands.SHOOT_COMMAND_NAME,
    // shootCommand(() -> false).withTimeout(5));
    // NamedCommands.registerCommand(Constants.NamedCommands.FEEDTHROUGH_COMMAND_NAME,
    // feedThroughCommand().withTimeout(2));
  ...
  private void configureBindings() {
    ...
    // PRIMARY_CONTROLLER.rightStick().getAsBoolean()));
    ...
    // Y button - aim and shoot at speaker, regardless if shooting if speaker tag is
    // visible
    // PRIMARY_CONTROLLER.y().whileTrue(shootCommand(() -> true));

    // PRIMARY_CONTROLLER.povUp().whileTrue(SHOOTER_SUBSYSTEM.shootManualCommand(()
    // -> dashboardStateSupplier()));
    ...
    PRIMARY_CONTROLLER.b().whileTrue(CLIMBER_SUBSYSTEM.lowerRightCommand());
    PRIMARY_CONTROLLER.y().whileTrue(CLIMBER_SUBSYSTEM.raiseRightCommand());
  }

  /**
   * Compose command to control controller rumble.
   * <ul>
   * <li>If the vision subsystem detects a game piece, the left side of the
   * controller will rumble
   * <li>If the intake has a game piece inside, the right side of the controller
   * will rumble
   * <li>Otherwise, no rumble :(
   * </ul>
   *
   * @return Command that will automatically make the controller rumble based on
  ...
  // }).finallyDo(() ->
  // PRIMARY_CONTROLLER.getHID().setRumble(RumbleType.kBothRumble, 0.0));
  // }

  /**
   * Compose command to intake a note
   *
   * @return Command that will automatically intake a note and prepare it for
   *         feeding inside the shooter motor
   */
  ...
  /**
   * Compose command to shoot note, checking if tag is visible and robot is in
   * range
   *
   * @return Command that will automatically aim and shoot note
   */
  ...
  /**
   * Command to aim at detected game object automatically, driving normally if
   * none is detected
   *
